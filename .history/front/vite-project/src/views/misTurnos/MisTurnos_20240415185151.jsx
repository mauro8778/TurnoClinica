...
const MisTurnos = () => {
  ...
  const handleAgendarTurno = async (event) => {
    ...
    try {
      ...
      setTurnos([...turnos, response.data]); // Agregar el nuevo turno a la lista de turnos
      // Limpiar los campos del formulario después de agregar un nuevo turno
      setDate("");
      setTime("");
      setDescription("");
    } catch (error) {
      setError('Error al agendar turno. Inténtalo de nuevo más tarde.'); // Mostrar mensaje de error al usuario
    }
  };
