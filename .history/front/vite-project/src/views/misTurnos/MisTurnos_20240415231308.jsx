...
const MisTurnos = () => {
    ...
    const userId = useSelector(state => state.auth.user?.id); // Obtiene el ID de usuario del estado global
  
    useEffect(() => {
      const fetchUserTurnos = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`http://localhost:3000/appoiment/user/${userId}`); // Modifica la URL para incluir el ID de usuario
