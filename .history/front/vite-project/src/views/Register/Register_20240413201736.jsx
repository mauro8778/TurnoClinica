  ...
  const handleFormSubmit = async (event) => {
    ...
    if (
      ...
      try {
        ...
        const existingPasswordResponse = await axios.get(
          `http://localhost:3000/users?password=${userData.password}`
        );
        if (existingPasswordResponse.data.length > 0) {
          setErrors((prevErrors) => ({
            ...prevErrors,
            password: "La contraseña ya está en uso"
          }));
          return;
        }
