export const isDiaSemana = (date) => {
    const dayOfWeek = date.getDay();
    const today = new Date().getDate();
    const selectedDay = date.getDate();
    
    
    return dayOfWeek >= 1 && dayOfWeek <= 5 && selectedDay > today;
};
  
export const isHoraValida = (date) => {
...
const validarTurno = (fecha) => {
    try {
        ...
        if (fechaTurno.getFullYear() < añoActual) {
            throw new Error(`Los turnos solo pueden ser agendados a partir de ${añoActual}.`);
        }
