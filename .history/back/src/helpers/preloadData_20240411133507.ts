...
const preLoadAppointmens = [
  ...
  export const PreLoadCredentials = async () => {
    await AppDataSource.manager.transaction(
      async (transactionalEntityManager) => {
        for await (const appoiments of preLoadAppointmens) {
          const newappointment = await AppDataSource.getRepository(appoiments).create(appoiments);
          await transactionalEntityManager.save(newappointment);
          const user = await userModel.findOneBy({ id: appointments.id });
          if (user) {
