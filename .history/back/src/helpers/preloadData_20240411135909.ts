import { AppDataSource, appoimentModel, userModel } from "../config/data-source";
    ...
    await AppDataSource.manager.transaction(
      async (transactionalEntityManager) => {
        for await (const appointment of preLoadAppointmens) {
          const newappointment = await appoimentModel.create(appoiments);
          await transactionalEntityManager.save(newappointment);
          const user = await userModel.findOneBy({ id: appointment });
          if (user) {
